...
class ProductDetail extends Component {
  ...
  async render() {
    ...
    this.view.btnFav.onclick = this._handleFavorites.bind(this);
  ...
  private async _handleFavorites() {
    if (!this.product) return;
    ...
    if (isInFavorites) {
      ...
      this.view.btnFav.innerHTML = '<svg class="svg-icon"><use href="#heart"></use></svg>'
    } else {
      favoritesService.addProduct(this.product);
      this.view.btnFav.innerHTML = '<svg class="svg-icon"><use href="#heart-filled"></use></svg>'
    }
  }
}
